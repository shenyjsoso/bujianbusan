...
@interface SYJInterestVC ()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout>
@property(nonatomic,strong)UICollectionView *CollectionView;

@end

@implementation SYJInterestVC
...
-(void)finish
{
    if (self.interestStr.length>0) {
        NSRange range1 = {0, 1};
        if ([[self.interestStr substringWithRange:range1] isEqualToString:@","]) {
            [self.interestStr deleteCharactersInRange:range1];
        }
        NSArray *array = [self.interestStr componentsSeparatedByString:@","];
        if (array.count>8) {
    ...
    NSDictionary *parameters = @{@"interest":self.interestStr};
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
 ...
 NSArray *array = [self.interestStr componentsSeparatedByString:@","];
    for (NSString *str in array) {
        if ([str isEqualToString:self.interestArr[indexPath.item]]) {
            cell.isAdd=YES;
            break;
        }
    }
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGSize size=[self sizeWithText:self.interestArr[indexPath.item] font:[UIFont systemFontOfSize:18]];
    size.height+=5;
    size.width+=20;
    return size;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell.isAdd==NO) {
        ...
        [self.interestStr appendString:[NSString stringWithFormat:@",%@", self.interestArr[indexPath.item]]];

        //NSLog(@"兴趣爱好:%@",self.interestStr);
    }
    else if (cell.isAdd==YES)
    {
        ...
        cell.interestLbl.textColor=SYJColor(152, 152, 152, 1);
        cell.layer.borderColor=SYJColor(152, 152, 152, 1).CGColor;
         NSRange substr = [self.interestStr rangeOfString:[NSString stringWithFormat:@",%@",self.interestArr[indexPath.item]]];
        while (substr.location != NSNotFound) {
            [self.interestStr deleteCharactersInRange:substr];
            substr = [self.interestStr rangeOfString:[NSString stringWithFormat:@",%@",self.interestArr[indexPath.item]]];
        }
    }
