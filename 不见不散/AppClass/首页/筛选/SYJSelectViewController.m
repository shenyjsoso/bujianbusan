...
#import "SYJFindSelect.h"
#import "SYJAllTags.h"
@interface SYJSelectViewController ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic, strong) UITableView  *tableView;
@property(nonatomic,strong) NSArray *basicArrs;
@property(nonatomic,strong)SYJFindSelect *infoArrs;
@property(nonatomic,strong)NSMutableArray *distanceArrs;
...
@implementation SYJSelectViewController
...
//    NSString *value=[NSString stringWithFormat:@"Bearer %@",[SYJTool getforkey:@"accessToken"]];
    [HttpTool getWithPath:API_FindSelect params:nil hearder:nil success:^(id json) {
        
        self.infoArrs=[SYJFindSelect mj_objectWithKeyValues:json];
...
-(void)initUI
{
    UIBarButtonItem *right=[[UIBarButtonItem alloc]initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(finishSelect)];
...
{
    return [self.basicArrs[section] count];
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    UIView* customView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 24)];
    // create the button object
    customView.backgroundColor= SYJColor(248, 248, 248, 1);
    UILabel * headerLabel = [[UILabel alloc] initWithFrame:CGRectZero];
    headerLabel.backgroundColor = [UIColor clearColor];
    headerLabel.opaque = NO;
    headerLabel.textColor =SYJColor(167, 167, 167, 1);
    headerLabel.highlightedTextColor = [UIColor whiteColor];
    headerLabel.font = [UIFont boldSystemFontOfSize:14];
    headerLabel.frame = CGRectMake(10, 0, ScreenWidth-10, 24);
    // If you want to align the header text as centered
    // headerLabel.frame = CGRectMake(150.0, 0.0, 300.0, 44.0);
    if (section==0) {
        headerLabel.text = @"基本筛选";
    }
    else{
        headerLabel.text = @"高级筛选";
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString * cellSYJSelectVC=@"idSYJSelectViewController";
    UITableViewCell * cell=[tableView dequeueReusableCellWithIdentifier:cellSYJSelectVC];
    if (!cell) {
        cell=[[UITableViewCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:cellSYJSelectVC];
    }
    //取消选中状态
    cell.selectionStyle =UITableViewCellSelectionStyleNone;
    //箭头
    cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    //cell.detailTextLabel.text = self.infoArrs[indexPath.section][indexPath.row];
    cell.textLabel.text = self.basicArrs[indexPath.section][indexPath.row];
    cell.textLabel.textColor=SYJColor(51, 51, 51, 1);
    if (indexPath.section==0) {
        switch (indexPath.row) {
            case 0:
                if ([self.infoArrs.distance isEqualToString:@"不限"]) {
                    cell.detailTextLabel.text=self.infoArrs.distance;
                }
                else
                {
                cell.detailTextLabel.text=[self.infoArrs.distance stringByAppendingString:@"km以内"];
                }
                break;
            case 1:
            ...
            case 3:
                ...
                break;
            case 4:
                if ([self.infoArrs.blood isEqualToString:@"不限"]) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //城市滚轮
    if (
        (indexPath.section==1&&indexPath.row==0)||(indexPath.section==0&&indexPath.row==5)
        ) {
...
//                NSString *value=[NSString stringWithFormat:@"Bearer %@",[SYJTool getforkey:@"accessToken"]];
                [HttpTool putWithPath:API_FindSelect params:params hearder:nil success:^(id json) {
                    //NSLog(@"修改成功%@",json);
                } failure:^(NSError *error) {
                    if (error.code==-1011) {
                        NSDictionary *errorDict=[SYJTool code1011:error];
...
//                NSString *value=[NSString stringWithFormat:@"Bearer %@",[SYJTool getforkey:@"accessToken"]];
    ...
    else
    {
        ...
        if(indexPath.section==0)
        {
            switch (indexPath.row) {
                case 0:
                    Pick.dataSource=self.distanceArrs;
                    break;
                case 4:
        ...
        Pick.callBlock = ^(NSString *pickDate) {
            if (pickDate) {
                ...
                if (indexPath.section==0 && indexPath.row==0)
                {
                    ...
                    {
                    celler.detailTextLabel.text=[pickDate stringByAppendingString:@"km以内"];}
                    parameters = @{@"distance":pickDate};
                }
...
//                NSString *value=[NSString stringWithFormat:@"Bearer %@",[SYJTool getforkey:@"accessToken"]];
                [HttpTool putWithPath:API_FindSelect params:parameters hearder:nil success:^(id json) {
                   // NSLog(@"修改成功%@",json);
...
{
    if (_basicArrs==nil) {
        _basicArrs=@[@[@"距离",@"年龄",@"身高",@"体重",@"血型",@"现居地"],@[@"户籍",@"年收入",@"学历",@"职业",@"婚史",@"购车情况",@"购房情况"]];
    }
    return _basicArrs;
}
...
-(NSMutableArray*)distanceArrs
{
    if (_distanceArrs==nil) {
        _distanceArrs=[NSMutableArray arrayWithObjects:@"不限", nil];
        for (NSInteger i=3; i<41; i++) {
            [_distanceArrs addObject:[NSString stringWithFormat:@"%ld",i]];
        }
    }
    return _distanceArrs;
}
