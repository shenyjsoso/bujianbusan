            ...
            if (flag) {
                ...
                if (_dataSource && [_dataSource respondsToSelector:@selector(emotionURLFormessageViewController:messageModel:)]) {
                    ...
                    if (emotion) {
                        ...
                        //NSLog(@"动画图%@",emotion.emotionOriginal);
                        //NSData *data = [NSData dataWithContentsOfFile:path];
                        //model.image=[UIImage sd_animatedGIFWithData:UIImagePNGRepresentation([UIImage imageNamed:emotion.emotionOriginal])];
...
- (void)_updateMessageStatus:(EMMessage *)aMessage
{
    ...
    if (aMessage && isChatting) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:modelForMessage:)]) {
            ...
            //model.avatarImage = [UIImage imageNamed:@"EaseUIResource.bundle/user"];
            [UserCacheManager getUserInfo:aMessage.from completed:^(UserCacheInfo *userInfo) {
                if (userInfo.avatarUrl) {
                    model.avatarURLPath=userInfo.avatarUrl;
                    model.avatarImage = [UIImage imageNamed:@"默认头像"];
                }
