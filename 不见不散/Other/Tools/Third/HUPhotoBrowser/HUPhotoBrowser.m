...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    _currentPage = scrollView.contentOffset.x/ScreenWidth + 0.5;
...
- (void)reloadForScreenRotate {
    ...
    self.collectionView.contentOffset = CGPointMake(ScreenWidth * _currentPage,0);
}
...
- (void)animateImageViewAtIndex:(NSInteger)index {
    ...
    if (self.imageView.image) {
        ...
        if (ratio > kScreenRatio) {
            ...
            endFrame.size.width = ScreenWidth;
            endFrame.size.height = ScreenWidth / ratio;
            
        } else {
            endFrame.size.height = ScreenHeight;
            endFrame.size.width = ScreenHeight * ratio;
            
        }
        endFrame.origin.x = (ScreenWidth - endFrame.size.width) / 2;
        endFrame.origin.y = (ScreenHeight - endFrame.size.height) / 2;
    ...
    [self.collectionView setContentOffset:CGPointMake(ScreenWidth * index,0) animated:NO];
...
- (void)imageSavedToPhotosAlbum:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    ...
    if(error != nil){
       ...
       // [SVProgressHUD showInfoWithStatus:msg];
        [SYJProgressHUD showMessage:msg inView:self afterDelayTime:1];
    }
    else{
        msg = @"保存图片成功";
